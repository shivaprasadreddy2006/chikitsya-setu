    ...
    try {
        ...
        enhancedLabs.forEach((lab, idx) => {
            ...
            if (lab.sampleCollectedAt) {
                timeline.push({
                    ...
                    details: `Sample received and barcoded by Pathology Technician in ${lab.labRoom}. Analysis in progress. ${lab.photoProof ? '📸 Photographic proof attached.' : ''}`,
            ...
            if (lab.status === 'REPORT_READY') {
                timeline.push({
                    ...
                    details: `Clinical Findings: "${lab.findings || 'Normal physiological ranges maintained'}". Reference: "${lab.referenceRange || 'Standard clinical limits'}". Delivery: ${lab.deliveryMode === 'PHYSICAL_COUNTER' ? '📄 Physical copy ready at Room 105' : '⚡ Verified Digital EHR Upload'}. ${lab.photoProof ? '📸 Diagnostic photo proof attached.' : ''}`,
        ...
        enhancedPrescriptions.forEach((rx, idx) => {
            ...
            if (rx.status === 'COMPLETELY_DISPENSED' || rx.status === 'DISPENSED' || rx.dispensedAt) {
                timeline.push({
                    ...
                    details: `All ${rx.medicines.length} medications dispensed at Counter #3. Zero cash payment verification complete. ${rx.photoProof ? '📸 Photographic proof of medicine packet attached.' : ''}`,
        ...
        if (admission) {
            timeline.push({
                ...
                details: `Admitted by ${admission.admittingDoctorName || admission.admittingDoctorId}. Allocated Bed: ${admission.bedNumber}. Diagnosis: ${admission.diagnosis}`,
                performedBy: admission.admittingDoctorName || 'Admitting Physician',
            ...
            if (admission.resourcesAllocated && admission.resourcesAllocated.length > 0) {
                admission.resourcesAllocated.forEach((res, i) => {
                    timeline.push({
                        ...
                        details: `Quantity: ${res.quantity}. Logged to patient bed ledger by ${res.loggedByStaff} (Zero Leakage audit trail). ${res.photoProof ? '📸 Bedside photo proof attached.' : ''}`,
            ...
            if (admission.status === 'DISCHARGED' || admission.dischargedAt) {
                timeline.push({
                    ...
                    performedBy: admission.dischargedByDoctorName || 'Ward Sister & Chief Resident',
                    room: admission.bedNumber,
                    block: admission.wardType,
                    status: 'DISCHARGED',
                    badgeBg: '#dcfce7',
    ...
    try {
        ...
        labRequests.forEach(l => {
            ...
            if (l.status === 'REPORT_READY') {
                allLogs.push({
                    ...
                    details: `Patient: ${l.patientId} | Findings: "${l.findings || 'Normal'}" | Mode: ${l.deliveryMode || 'DIGITAL_EHR'} ${l.photoProof ? '📸 [Photo Proof Attached]' : ''}`,
                    actor: 'Pathology Lab In-Charge',
                    patientId: l.patientId,
                    photoProof: l.photoProof,
                    status: 'PUBLISHED',
                    color: '#15803d'
                });
            }
        ...
        prescriptions.forEach(rx => {
            ...
            if (rx.status === 'COMPLETELY_DISPENSED' || rx.status === 'DISPENSED' || rx.dispensedAt) {
                allLogs.push({
                    type: 'PHARMACY_DISPENSE',
                    title: `Free Medications Dispensed ${rx.photoProof ? '📸 [Photo Proof Verified]' : ''}`,
                    timestamp: rx.dispensedAt || rx.updatedAt,
                    details: `Patient: ${rx.patientId} | Counter #3 | Zero Cash charge policy applied ${rx.photoProof ? '📸 Photographic handover proof attached.' : ''}`,
        ...
        admissions.forEach(adm => {
            allLogs.push({
                ...
                details: `Patient: ${adm.patientId} (${adm.patientName || ''}) | Bed: ${adm.bedNumber} | Diagnosis: ${adm.diagnosis}`,
                actor: adm.admittingDoctorName || adm.admittingDoctorId,
            ...
            if (adm.resourcesAllocated && adm.resourcesAllocated.length > 0) {
                adm.resourcesAllocated.forEach(res => {
                    allLogs.push({
                        type: 'RESOURCE_USAGE',
                        title: `Micro-Resource Logged: ${res.itemName} ${res.photoProof ? '📸 [Bedside Photo Proof]' : ''}`,
                        timestamp: res.loggedAt || adm.createdAt,
                        details: `Patient: ${adm.patientId} (${adm.patientName || ''}) | Qty: ${res.quantity} | Logged by: ${res.loggedByStaff} (Anti-Theft Ledger) ${res.photoProof ? '📸 Bedside proof attached.' : ''}`,
            ...
            if (adm.status === 'DISCHARGED' || adm.dischargedAt) {
                allLogs.push({
                    ...
                    details: `Patient: ${adm.patientId} (${adm.patientName || ''}) | Summary: "${adm.dischargeSummary || 'Stable'}"`,
                    actor: adm.dischargedByDoctorName || 'Ward Sister',
                    patientId: adm.patientId,
                    status: 'COMPLETED',
                    color: '#166534'
                });
